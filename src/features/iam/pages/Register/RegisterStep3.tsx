...
const RegisterStep3: React.FC<RegisterStep3Props> = ({ onSubmit, onPrevious }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedGames, setSelectedGames] = useState<string[]>([]);

  // Dados simulados de jogos e categorias - SUBSTITUA PELOS SEUS DADOS REAIS OU CHAME UMA API
  const allGames = [
    { title: 'Harvest Moon', category: 'Simulação', imageUrl: 'https://via.placeholder.com/100x100/6B40E3/FFFFFF?text=HM' },
    { title: 'Farmville', category: 'Simulação', imageUrl: 'https://via.placeholder.com/100x100/6B40E3/FFFFFF?text=FV' },
    { title: 'Animal Crossing', category: 'Simulação', imageUrl: 'https://via.placeholder.com/100x100/6B40E3/FFFFFF?text=AC' },
    { title: 'Stardew Valley', category: 'Simulação', imageUrl: 'https://via.placeholder.com/100x100/6B40E3/FFFFFF?text=SDV' },
    { title: 'Terraria', category: 'Aventura', imageUrl: 'https://via.placeholder.com/100x100/6B40E3/FFFFFF?text=TR' },
    { title: 'Minecraft', category: 'Aventura', imageUrl: 'https://via.placeholder.com/100x100/6B40E3/FFFFFF?text=MC' },
    { title: 'Valheim', category: 'Sobrevivência', imageUrl: 'https://via.placeholder.com/100x100/6B40E3/FFFFFF?text=VH' },
    { title: 'Among Us', category: 'Casual', imageUrl: 'https://via.placeholder.com/100x10E3/FFFFFF?text=AU' },
  ...
  const filteredAndGroupedGames = React.useMemo(() => {
    ...
    return filtered.reduce((acc, game) => {
      ...
      return acc;
    }, {} as Record<string, typeof allGames>); // Tipagem para o objeto agrupado
  }, [searchTerm, allGames]);
  ...
  return (
    <div className="bg-[#2B2156] mt-16 mb-32 p-8 md:p-10 rounded-2xl shadow-2xl w-full max-w-lg mx-auto overflow-hidden">
      ...
      <div className="flex mb-8 flex-col">
        ...
        <button onClick={onPrevious} className="text-white text-3xl self-baseline mt-1">
          &larr;
        </button>
      </div>
      ...
      <div className="max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
        {Object.entries(filteredAndGroupedGames).map(([categoryName, games]) => (
          <div key={categoryName} className="mb-6 last:mb-0">
            <h2 className="text-white text-xl font-semibold mb-3">{categoryName}</h2>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
              {games.map((game, gameIndex) => (
                ...
                >
                  ...
                  <div className={`absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 p-1 text-center text-white text-xs font-semibold
