...
const CollectionPage: React.FC = () => {
	...
	useEffect(() => {
		...
		tempGames = tempGames.filter((game) => {
			...
			const matchesDate =
				gameYear >= filters.dateRange[0] && gameYear <= filters.dateRange[1];
			const matchesPlatforms =
				filters.platforms.length === 0 ||
				filters.platforms.some((fp) => game.platform.includes(fp));
			const matchesGenres =
				filters.genres.length === 0 ||
				filters.genres.some((fg) => game.genre.includes(fg));
			const matchesDevelopers =
				filters.developers.length === 0 ||
				filters.developers.some((fd) => game.developer.includes(fd));
			return (
				matchesDate && matchesPlatforms && matchesGenres && matchesDevelopers
			);
		});
		...
		<div className="min-h-screen bg-[#1C1C2E] text-white font-jost">
			<div className="max-w-7xl mx-auto p-8 pt-4 md:p-10">
				...
				<p className="text-white text-opacity-70 text-lg mb-8">
					Explore sua coleção. Filtre por plataforma, gênero ou ano de
					lançamento.
				</p>
						...
						<div className="mb-2 pt-4 pb-4 border-b border-gray-700">
							...
							{showPlatforms && (
								<div className="mt-4">
									...
									<button className="text-[#CBE220] text-sm mt-2 hover:underline">
										Mostrar mais
									</button>
								</div>
							)}
						...
						<div className="pt-4">
							...
							{showDevelopers && (
								<div className="mt-4">
									{" "}
									{/* Removido text-center daqui */}
									{["Garena", "PUBG", "Corporation", "Activision"].map(
										(developer) => (
											<label
												key={developer}
												className="flex items-center text-white mb-2"
					...
					<div className="lg:col-span-3">
						<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
							{currentGames.map((game) => (
								<div
									key={game.id}
									className="bg-[#ECE9F7] rounded-xl shadow-lg overflow-hidden flex"
								>
									{/* Imagem do jogo */}
							...
							{Array.from({ length: totalPages }).map((_, i) => (
								<button
									...
									className={`px-4 py-2 rounded-lg font-semibold ${
										currentPage === i + 1
											? "bg-[#6B40E3] text-white"
											: "bg-[#42397B] text-gray-300 hover:bg-[#5734B7]"
									}`}
								>
									{i + 1}
								</button>
